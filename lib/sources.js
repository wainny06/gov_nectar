...
export function category(title) {return /보증|금융|자금|이차보전/.test(title)?'자금·보증':/바우처/.test(title)?'바우처':/수출|해외|물류|바이어/.test(title)?'수출·해외판로':/마케팅|광고|홍보/.test(title)?'마케팅':/판로|입점|판매|유통/.test(title)?'국내판로':'기타 지원';}
export function parseLinks(html, source) {
 ...
 for(const m of html.matchAll(re)) {
  ...
  let url;try{url=new URL(href.replace(/&amp;/g,'&'),source.url);if(!/^https?:$/.test(url.protocol)||url.hostname!==new URL(source.url).hostname)continue;}catch{continue;}
  url.hash='';map.set(url.href,{id:url.href,title,url:url.href,source:source.id,category:category(title),support:'원문 확인 필요',conditions:'원문 확인 필요',deadline:'',deadlineTime:'',status:'미검토',note:'',checkedAt:new Date().toISOString(),verification:'제목 수집 · 상세 확인 필요'});
 }
