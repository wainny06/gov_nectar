...
async function sync(){
 ...
 try{
  ...
  for(const r of changed){status.textContent=`Notion 업데이트 ${done}/${changed.length} · 창을 열어 두세요.`;const response=await fetch('/api/notion-sync',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${key.value}`},body:JSON.stringify(r)});const value=await response.json();if(!response.ok)throw Error(value.error||'동기화 실패');sent.set(r.key,JSON.stringify(r));done++;}
 ...
 finally{busy=false;button.disabled=false;if(pending){pending=false;schedule();}}
}
...
fetch('/api/notion-sync').then(r=>{if(!r.ok)throw Error();return r.json();}).then(r=>{status.textContent=r.configured?'연결 설정 있음 · 키 입력 후 업데이트를 실행해 주세요.':'설정 대기 · Vercel에 NOTION_TOKEN, NOTION_SYNC_SECRET 등록 후 재배포가 필요합니다.';}).catch(()=>{status.textContent='연결 상태를 확인하지 못했습니다. 배포 상태를 확인해 주세요.';});
